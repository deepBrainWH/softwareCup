...
def build_model():
    ...
    with tf.variable_scope("conv_layer_1"):
        conv1 = tf.layers.conv2d(\
            x, 64, [3, 3], activation=tf.nn.relu, name='conv1')
    ...
    with tf.variable_scope("conv_layer_2"):
        conv2 = tf.layers.conv2d(\
            output1, 64, [3, 3], activation=tf.nn.relu, name='conv2')
    ...
    with tf.variable_scope("conv_layer_3"):
        conv3 = tf.layers.conv2d(\
            output2, 64, [3, 3], activation=tf.nn.relu, name='conv3')
    ...
    with tf.variable_scope("conv_layer_4"):
        conv4 = tf.layers.conv2d(\
            output3, 32, [3, 3], activation=tf.nn.relu, name='conv4')
    ...
    with tf.variable_scope("fc_layer3"):
        fc3 = tf.layers.dense(dropout2, 4)
